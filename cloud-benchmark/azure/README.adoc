...
To clear up the storage used by the XTDB node (ie, the contents of the blob storage), you can run the following script:

```
./scripts/clear-azure-storage.sh
```

NOTE: This is currently required between runs when using remote object storage and Local Directory based logs.
